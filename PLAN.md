...
16. **Neu (2026-09-25), Feature "Listen" – in Arbeit.** Erste der vier Kacheln bekommt echte Funktion. Entwurf (nach Vorbild eines Reminders/Listen-Apps): Listen-Übersichtsseite `Lists.php` mit farbigen Kacheln je Liste (Icon, Name, Anzahl Einträge/"Leer"), "+"-Button für neue Liste. Pro Liste: eigene Seite mit Einträgen (Text + Abhaken), 3-Punkte-Menü (bearbeiten/umbenennen, löschen, Zugriff verwalten, erledigte ausblenden). Teilen nur innerhalb der eigenen Familie, wer Zugriff hat darf alles (keine abgestuften Rechte). Live-Sync zwischen Geräten (Polling) **bewusst zurückgestellt** – erst Kernfunktion mit normalem Neuladen, Polling später als Erweiterung.
    - **DB fertig, getestet (2026-09-25):** `lists` (`fam_id`, `owner_id`, `name`), `list_access` (n:m Liste↔User, UNIQUE `list_id`+`user_id` – wer nur den Ersteller drin hat, ist "privat"), `list_items` (`list_id`, `content`, `done` TINYINT(1) Default 0). Spalte bewusst `content` statt `text` genannt (TEXT ist in MySQL ein Datentyp, hätte verwirrt). Insert/UNIQUE/Cascade-Delete in Transaktion getestet, alles korrekt.
    - **Kernfunktion fertig, getestet (2026-09-25):** `FamilyList.php` (8 Methoden: createList, addAccess, hasAccess, getListsForUser, getListById, getItems, addItem, toggleItem – `toggleItem` per `SET done = NOT done` in einem SQL-Befehl statt lesen+umdrehen+schreiben). `CreateFamily.php` legt automatisch "Einkaufen"+"To-Dos" an. `Lists.php` (Übersicht als Kacheln) + `ListDetail.php` (einzelne Liste: Einträge, hinzufügen, abhaken per `?toggle=id` in der URL, Sicherheitscheck mit `hasAccess()`). "Listen"-Kachel im Dashboard verlinkt (`data-href="Lists.php"`, nutzt das bestehende `tiles.js` automatisch mit). Voller Ablauf getestet: neue Familie → Standardlisten da → Eintrag hinzufügen → abhaken → Zugriff auf fremde Liste wird korrekt verweigert.
    - **Noch offen:** "+"-Button für eigene neue Listen (mit Auswahl, wem geteilt wird), 3-Punkte-Menü (umbenennen, löschen, Zugriff verwalten, erledigte ausblenden), später Live-Sync per Polling
...
- 2026-09-25 (später, Feature "Listen" gestartet): Anforderungen besprochen (private/geteilte Listen nur innerhalb der Familie, wer Zugriff hat darf alles, Live-Sync bewusst auf später verschoben). Nach Reminders-App-Vorbild geplant: Übersichtsseite mit farbigen Listen-Kacheln + "+"-Button, Einzel-Listen-Seite mit 3-Punkte-Menü (bearbeiten, löschen, Zugriff verwalten, erledigte ausblenden). DB fertig: `lists`, `list_access` (n:m, UNIQUE), `list_items` (Spalte `content` statt `text`, da TEXT ein MySQL-Datentyp ist). Getestet (Transaktion): Insert, UNIQUE-Schutz, Cascade-Delete über zwei Ebenen (Liste löschen → list_access UND list_items verschwinden automatisch mit). Nächster Schritt: Standardlisten bei `CreateFamily.php`, dann DB-Klasse, dann `Lists.php`.
- 2026-09-25 (Kernfunktion "Listen" fertig): `FamilyList.php` mit acht Methoden gebaut (inkl. `toggleItem` per `SET done = NOT done`, kein Lesen+Umdrehen+Schreiben nötig). Beim Abtippen drei SQL-Fehler gefunden und behoben (INSERT mit WHERE statt VALUES, fehlende Klammer, UPDATE ohne Wert). `CreateFamily.php` legt jetzt automatisch "Einkaufen"+"To-Dos" an. `Lists.php` (Übersicht) und `ListDetail.php` (Einträge, hinzufügen, abhaken über `?toggle=id`, `hasAccess()`-Sicherheitscheck) gebaut. "Listen"-Kachel im Dashboard verlinkt. Voller End-to-End-Test über echten HTTP-Testserver (neue Familie → Standardlisten da → Eintrag → abhaken → Fremdzugriff verweigert), Testdaten wieder entfernt. Offen: "+"-Button für neue Listen, 3-Punkte-Menü, Polling für Live-Sync.
